...
(defun temp-magit-test-fun ()
(message "Custom magit fn loading.")
(require 'magit)
...
;; (setq vc-handled-backends (delq 'Git vc-handled-backends)) ;; NOTE: removed this line to enable vc
...
(evil-define-key nil magit-revision-mode-map (kbd "q") 'bury-buffer)
...
(defun init-magit-log-mode-keybindings ()
  (custom-apply-to-pairs `define-key evil-normal-state-local-map (list
								  (kbd ";gca") 'magit-commit-amend
								  (kbd ";gra") 'git-rebase-abort
								  (kbd ";grc") 'git-rebase-continue
								  (kbd ";gri") 'magit-rebase-interactive
								  (kbd ";gpush") 'git-push
								  (kbd ";gpull") 'git-pull
								  ;; (kbd "o") 'show-commit-and-preserve-window
								  ;; (kbd "RET") 'show-commit-and-preserve-window
								  (kbd "yy") 'magit-copy-section-value ; Copies the commit ID of the commit under the cursor.
								  (kbd "r") 'magit-refresh
								  (kbd "q") 'bury-buffer
								  ...
								  (kbd "u") (lambda () (interactive) (scroll-magit-revision-buffer 'View-scroll-half-page-backward))
								  (kbd "d") (lambda () (interactive) (scroll-magit-revision-buffer 'View-scroll-half-page-forward)))))
...
(defun init-magit-status-mode-keybindings ()
  ...
  (custom-apply-to-pairs `define-key evil-normal-state-local-map (list
								  ...
								  (kbd "e") 'magit-show-level-4-all ; e for exapnd
								  (kbd "d") 'magit-discard
								  (kbd "s") 'magit-stage
...
(defun init-magit-commit-mode-keybindings ()
  ...
  (evil-local-set-key 'normal (kbd "yy") 'evil-yank-line)
  (evil-local-set-key 'normal "y" 'evil-yank))
